...
function Get (db, key, opts, cb) {
  ...
  debug('get: %O, hash: %s', key, this._node.hashAsCrumbs() )
...
Get.prototype._update = function (i, head) {
  debug('walk: %d, key: %s', head.seq, head.hashAsCrumbs())
  ...
  for (; i < this._length; i++) {
    ...
    debug('chose bucket at depth %d: %O',i, bucket)
